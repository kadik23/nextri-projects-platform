...
////////
 ...
 const handleGithubLogin = async (e: React.FormEvent) => {
  ...
  return (
    <main className="h-screen flex flex-col pt-10 items-center gap-2  ">
      <h1 className="text-2xl sm:text-3xl font-semibold tracking-wider text-center">Welcome AT NEXTRI PROJECTS</h1>
      <p className="text-sm sm:text-base text-center">Sign in or create an account and start building projects!</p>
 ...
 <form className="space-y-4 my-5 w-[80%] max-w-xl" onSubmit={handleSubmit} >
            ...
            <Button type="submit" className="w-full disabled:cursor-not-allowed flex gap-2" disabled={isLoading}>
          ...
          <div className="space-y-2  w-[80%] max-w-xl">
            ...
            <Button variant="outline" className="w-full flex gap-2" disabled={isLoggingWithGoogle} onClick={handleGithubLogin} >
              {isLoggingWithGithub ? <MiniSpinner/> :<GithubIcon className=" h-4 w-4" />}
              Sign in with Github
            </Button>
            <Button variant="outline" className="w-full flex gap-2" disabled={isLoggingWithGoogle} onClick={handleGoogleLogin} >
              {isLoggingWithGoogle ? <MiniSpinner/> :<ChromeIcon className=" h-4 w-4" />}
              Sign in with Google
            </Button>
          </div>
...
const GithubIcon: FC<React.SVGProps<SVGSVGElement>> = (props) => (
  <svg {...props} role="img" width="20" height="20" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
